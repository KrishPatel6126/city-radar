...
function internetSearch() {
    ...
    li.innerHTML = `
        ...
        <div class="meta">Open results in Google Maps</div>
    `;
...
/* LOCAL PUBLIC RECOMMENDED (WITH BUDGET FILTER) */
function localSearch() {
    ...
    filtered.forEach(p => {
        ...
        li.onclick = () => {
            window.open(
                `https://www.google.com/maps/search/${encodeURIComponent(p.name + " " + p.area)}`,
                "_blank"
            );
        };
